...
class DualEncoderFusionBlock(nn.Module):
    """Bidirectional attention fusion for dual encoders"""
    def __init__(self, channels, reduction=8):
        super(DualEncoderFusionBlock, self).__init__()
        
        # ResNet guides Swin (ResNet as gate, Swin as input)
        self.attn_res_to_swin = AttentionBlock(
            F_g=channels,  
            F_l=channels,  
            F_int=channels // reduction
        )
    ...
    def forward(self, res_feat, swin_feat):
        ...
        swin_attended = self.attn_res_to_swin(g=res_feat, x=swin_feat)
        res_attended = self.attn_swin_to_res(g=swin_feat, x=res_feat)
        
        # Concatenate attended features
        fused = torch.cat([res_attended, swin_attended], dim=1)
        
        return fused

class WeightedAttentionDualEncoderModel(nn.Module):
    def __init__(self, out_channels=1, freeze_encoders=True):
        super(WeightedAttentionDualEncoderModel, self).__init__()
        ...
        self.fusion_attn4 = DualEncoderFusionBlock(512, reduction=8)  # 12×12
        self.fusion_attn3 = DualEncoderFusionBlock(256, reduction=8)  # 24×24
        self.fusion_attn2 = DualEncoderFusionBlock(128, reduction=8)  # 48×48
        self.fusion_attn1 = DualEncoderFusionBlock(128, reduction=8)  # 96×96

        # Decoder blocks (in_channels, skip_channels, out_channels)
        self.up1 = DecoderBlock(1024, 512, 512)  # 12×12 → 24×24
        self.up2 = DecoderBlock(512, 256, 256)   # 24×24 → 48×48
        self.up3 = DecoderBlock(256, 256, 128)   # 48×48 → 96×96
...
if __name__ == "__main__":
    model = WeightedAttentionDualEncoderModel(out_channels=1)
