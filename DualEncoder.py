...
class DualEncoderModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # Fuse encoder features via concatenation
        concat_4 = torch.cat([res_enc4, swin_enc4], dim=1)  # 512+512 = 1024 @ 12×12
        concat_3 = torch.cat([res_enc3, swin_enc3], dim=1)  # 256+256 = 512  @ 24×24
        concat_2 = torch.cat([res_enc2, swin_enc2], dim=1)  # 128+128 = 256  @ 48×48
        concat_1 = torch.cat([res_enc1, swin_enc1], dim=1)  # 128+128 = 256  @ 96×96
...
if __name__ == "__main__":
    DualEncoderModel = DualEncoderModel(out_channels=1)
    input_tensor = torch.randn(1, 3, 384, 384)
    output = DualEncoderModel(input_tensor)
    print(output.shape)
