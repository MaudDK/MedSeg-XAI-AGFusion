...
class DualEncoderFusionBlock(nn.Module):
    ...
    def forward(self, res_feat, swin_feat):
        ...
        res_attended = self.attn_swin_to_res(g=res_feat, x=swin_feat)
        return res_attended

class AttentionDualEncoderRes(nn.Module):
    def __init__(self, out_channels=1, freeze_encoders=True):
        super(AttentionDualEncoderRes, self).__init__()
...
if __name__ == "__main__":
    model = AttentionDualEncoderRes(out_channels=1)
